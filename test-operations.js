...
function TestOperations (page) {
  this.assertItemInputFocused = function () {
    return page.waitForFocusedElement(
      page.getEditingListItemInputCss(),
  ...
  this.assertClearCompleteButtonText = function (buttonText) {
    return page
      ...
      .then(
        page.waitForTextContent.bind(
          page,
          buttonText,
          'Expected clear button text to be ' + buttonText
        )
      )
  }
  ...
  this.assertClearCompleteButtonVisibility = function (shouldBeVisible) {
    ...
    return page.waitForVisibility(
      shouldBeVisible,
      page.getClearCompletedButtonCss(),
      failMsg
    )
  }
  ...
  this.assertMainSectionVisibility = function (shouldBeVisible) {
    var failMsg =
      'Expected main section to be ' + (shouldBeVisible ? 'visible' : 'hidden')
    return page.waitForVisibility(
      shouldBeVisible,
      page.getMainSectionCss(),
      failMsg
    )
  }
  ...
  this.assertFooterVisibility = function (shouldBeVisible) {
    var failMsg =
      'Expected footer to be ' + (shouldBeVisible ? 'visible' : 'hidden')
    return page.waitForVisibility(
      shouldBeVisible,
      page.getFooterSectionCss(),
      failMsg
    )
  }
  ...
  this.assertItemToggleIsHidden = function (index) {
    return page.waitForVisibility(
      false,
      page.getListItemToggleCss(index),
  ...
  this.assertItemLabelIsHidden = function (index) {
    return page.waitForVisibility(
      false,
      page.getListItemLabelCss(index),
  ...
  this.assertItemText = function (itemIndex, text) {
    return page
      .waitForItemLabel(itemIndex)
      .then(
        page.waitForTextContent.bind(
          page,
          text,
          'Expected the item label to be ' + text
        )
      )
  }
    ...
    return page.getListItems().then(function (items) {
      if (items.length < textArray.length) {
        ...
        textArray = textArray.filter(function (item) {
          return item !== page.ITEM_HIDDEN_OR_REMOVED
        })
      }
      var ret
      textArray.forEach(function (text, i) {
        if (text === page.ITEM_HIDDEN_OR_REMOVED) {
          return
        }
        var promise = page.waitForTextContent(
          text,
          'Expected item text to be ' + text,
          items[i]
        )
  ...
  this.assertItemCountText = function (text) {
    return page
      .waitForElement(page.getItemCountCss())
      .then(
        page.waitForTextContent.bind(
          page,
          text,
          'Expected item count text to be ' + text
        )
      )
  }
  ...
  this.assertCompleteAllCheckedStatus = function (shouldBeChecked) {
    var failMsg =
      'Expected the mark-all-completed checkbox to be ' + shouldBeChecked
        ? 'checked'
        : 'unchecked'
    return page
