...
const PageLaxMode = require('./page-lax-mode')
const TestOperations = require('./test-operations')
const colo = require('colo')

chrome.setDefaultService(new chrome.ServiceBuilder(chromedriver.path).build())

const onOffLabel = bool => bool ? '' : ''

/**
 * @param {string} baseUrl
 * @param {boolean} speedMode
 * @param {boolean} laxMode
 * @param {string} browserName
 */
module.exports.todoMVCTest = (
  baseUrl,
  speedMode,
  laxMode,
  browserName
) => {
  console.log(colo.magenta(`Starting the test of TodoMVC app on ${baseUrl}`))
  console.log()
  console.log('Options:')
  console.log(colo.cyan(`- speedMode: ${!!speedMode}`))
  console.log(colo.cyan(`- laxMode: ${!!laxMode}`))

  test.describe('TodoMVC', function () {
    const TODO_ITEM_ONE = 'buy some cheese'
    const TODO_ITEM_TWO = 'feed the cat'
    const TODO_ITEM_THREE = 'book a doctors appointment'
    let browser, testOps, page
    ...
    function createStandardItems (done) {
      ...
      return page.enterItem(TODO_ITEM_THREE).then(function () {
    ...
    function launchBrowser (done) {
      ...
      return page.ensureAppIsVisibleAndLoaded().then(function () {
        if (done instanceof Function) {
          done()
        }
      })
    ...
    function printCapturedLogs () {
      ...
      return logs.get('browser').then(function (entries) {
    ...
    test.describe('When page is initially opened', function () {
      test.it('should focus on the todo input field', function (done) {
        testOps.assertNewInputFocused().then(function () {
          done()
        })
      })
    })
    ...
    test.describe('No Todos', function () {
      test.it('should hide #main and #footer', function (done) {
        ...
        testOps.assertFooterVisibility(false).then(function () {
          done()
        })
      })
    })
    ...
    test.describe('New Todo', function () {
      test.it('should allow me to add todo items', function (done) {
        ...
        page.enterItem(TODO_ITEM_TWO)
        testOps.assertItems([TODO_ITEM_ONE, TODO_ITEM_TWO]).then(function () {
          done()
        })
      })
      ...
      test.it('should clear text input field when an item is added', function (
        ...
        page.enterItem(TODO_ITEM_ONE)
        testOps.assertNewItemInputFieldText('').then(function () {
          done()
        })
      })
      ...
      test.it('should append new items to the bottom of the list', function (
        ...
        testOps.assertItemText(2, TODO_ITEM_THREE).then(function () {
          done()
        })
      })
      ...
      test.it('should trim text input', function (done) {
        page.enterItem('   ' + TODO_ITEM_ONE + '  ')
        testOps.assertItemText(0, TODO_ITEM_ONE).then(function () {
          done()
        })
      })
      ...
      test.it('should show #main and #footer when items added', function (done) {
        ...
        testOps.assertFooterVisibility(true).then(function () {
          done()
        })
      })
    })
    ...
    test.describe('Mark all as completed', function () {
      ...
      test.it('should allow me to mark all items as completed', function (done) {
        ...
        testOps.assertItemCompletedStates([true, true, true]).then(function () {
          done()
        })
      })

      test.it(
        'should correctly update the complete all checked state',
        function (done) {
          ...
          testOps.assertCompleteAllCheckedStatus(true).then(function () {
            done()
          })
        }
      )
      ...
      test.it(
        ...
        function (done) {
          ...
          testOps
            .assertItemCompletedStates([false, false, false])
            .then(function () {
              done()
            })
        }
    ...
    test.describe('Item', function () {
      test.it('should allow me to mark items as complete', function (done) {
        ...
        page.toggleItemAtIndex(1)
        testOps.assertItemCompletedStates([true, true]).then(function () {
          done()
        })
      })
      ...
      test.it('should allow me to un-mark items as complete', function (done) {
        ...
        page.toggleItemAtIndex(0)
        testOps.assertItemCompletedStates([false, false]).then(function () {
          done()
        })
      })
    })
    ...
    test.describe('Editing', function () {
      test.beforeEach(function (done) {
        createStandardItems()
        page.doubleClickItemAtIndex(1).then(function () {
          done()
        })
      })
      ...
      test.it('should focus the input', function (done) {
        ...
        testOps
          .assertNewInputBlurred() // Unnecessary? The HTML spec dictates that only one element can be focused.
          .then(function () {
            done()
          })
      })
      ...
      test.it('should hide other controls when editing', function (done) {
        ...
        testOps.assertItemLabelIsHidden(1).then(function () {
          done()
        })
      })
      ...
      test.it('should trim entered text', function (done) {
        page.editItemAtIndex(
          1,
          '    buy some sausages  ' + webdriver.Key.ENTER
        )
        testOps
          .assertItems([TODO_ITEM_ONE, 'buy some sausages', TODO_ITEM_THREE])
          .then(function () {
    ...
    test.describe('Counter', function () {
      test.it('should display the current number of todo items', function (
        ...
        page.enterItem(TODO_ITEM_TWO)
        testOps.assertItemCountText('2 items left').then(function () {
          done()
        })
      })
    })
    ...
    test.describe('Clear completed button', function () {
      ...
      test.it('should display the correct text', function (done) {
        page.toggleItemAtIndex(1)
        testOps
          .assertClearCompleteButtonText('Clear completed')
          .then(function () {
            done()
          })
      })
      ...
      test.it('should remove completed items when clicked', function (done) {
        ...
        testOps.assertItems([TODO_ITEM_ONE, TODO_ITEM_THREE]).then(function () {
          done()
        })
      })
      ...
      test.it(
        ...
        function (done) {
          ...
          page.clickClearCompleteButton()
          testOps.assertClearCompleteButtonVisibility(false).then(function () {
            done()
          })
        }
      )
    ...
    test.describe('Routing', function () {
      ...
      test.it('should highlight the currently applied filter', function (done) {
        ...
        page.filterByCompletedItems()
        testOps.assertFilterAtIndexIsSelected(2).then(function () {
          done()
        })
      })
    })
